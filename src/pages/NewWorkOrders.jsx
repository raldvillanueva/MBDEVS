    ...
    <div className="flex h-[calc(100vh-64px)] flex-col gap-4">
      ...
      <div className="min-h-0 flex-1 overflow-auto rounded-xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full text-sm"><thead className="sticky top-0 bg-slate-800 text-xs text-slate-300"><tr>{isAdmin && <th className="w-10 px-4 py-3 text-center font-medium"><input type="checkbox" checked={records.length > 0 && records.every(record => selectedIds.includes(record.id))} onChange={toggleSelectAll} /></th>}<th className="px-4 py-3 text-left font-medium">FIELD ORDER</th><th className="px-4 py-3 text-left font-medium">FO ACTION</th><th className="px-4 py-3 text-left font-medium">INSTALLED METER</th><th className="px-4 py-3 text-left font-medium">CREW NAME</th><th className="px-4 py-3 text-left font-medium">SERVICE ID NUMBER</th><th className="px-4 py-3 text-right font-medium">ACTION</th></tr></thead>
          <tbody>{loading ? <tr><td colSpan={isAdmin ? 7 : 6} className="px-4 py-16 text-center text-slate-400">Loading new work orders...</td></tr> : records.length === 0 ? <tr><td colSpan={isAdmin ? 7 : 6} className="px-4 py-16 text-center text-slate-400">No new work orders.</td></tr> : records.map(record => <tr key={record.id} onClick={() => openEdit(record)} className="cursor-pointer border-t border-slate-100 hover:bg-slate-50">{isAdmin && <td className="px-4 py-3 text-center" onClick={event => event.stopPropagation()}><input type="checkbox" checked={selectedIds.includes(record.id)} onChange={() => toggleSelect(record.id)} /></td>}<td className="px-4 py-3 font-mono text-blue-600">{record.field_order_no || '—'}</td><td className="px-4 py-3">{record.fo_action || '—'}</td><td className="px-4 py-3">{record.ins_meter || '—'}</td><td className="px-4 py-3">{record.crew_name || '—'}</td><td className="px-4 py-3">{record.service_number || '—'}</td><td className="px-4 py-3 text-right">{isAdmin && <button onClick={event => { event.stopPropagation(); completeWorkOrder(record) }} disabled={movingId === record.id} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-emerald-700 disabled:opacity-60"><CheckCircle size={14} /> {movingId === record.id ? 'Completing...' : 'Mark Completed'}</button>}</td></tr>)}</tbody>
        </table>
      </div>
